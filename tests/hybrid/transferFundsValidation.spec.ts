...
test("TC-11 : Transfer Funds via UI and Validate via API", async ({
  page,
  request,
}) => {
  ...
  const destinationBeforeBalance = extractBalance(await destinationBeforeResponse.text(),);
  ...
  const destinationAfterBalance = extractBalance(await destinationAfterResponse.text(),);

  console.log("Source After:", sourceAfterBalance);
  console.log("Destination After:", destinationAfterBalance);

  expect(sourceAfterBalance).toBeLessThan(sourceBeforeBalance);
  expect(destinationAfterBalance).toBeGreaterThan(destinationBeforeBalance);
});
